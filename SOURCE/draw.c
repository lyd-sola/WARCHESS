...
buffer为存储的数组
*****************************************/
void GetBackground(int left, int top, int right, int bottom, unsigned *buffer)//获取背景
{
    int i, j;
	unsigned co;
    int Width, Height;
...
void PutBackground(int left, int top, int right, int bottom, unsigned *buffer)//贴回背景
{     
    int i, j;    
    int Width, Height;     
...
void Icon_tank(POS pos, int side)
{
	...
	Bar64k(pos.x-7, pos.y-10, pos.x+7, pos.y+12, color1); //车身 
	Bar64k(pos.x-3, pos.y-10, pos.x+3, pos.y+12, color3); //炮塔
	Bar64k(pos.x-1, pos.y, pos.x+1, pos.y+18, color2);
	Circlefill64k(pos.x, pos.y, 5, color2); //炮管
}
...
int putbmp_partial(int x1, int y1, int x2, int y2, char *s)//前四个参数为贴图的矩形范围，仅能贴全屏大图
{
